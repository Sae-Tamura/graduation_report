...
\chapter{提案手法}
本章では，大規模言語モデル（LLM）を用いてシーングラフを生成する提案手法について述べる．
提案手法の特徴は，画像ごとに関係を一から生成するのではなく，LLMが学習した常識的知識に基づいて
\textbf{物体間関係の知識をあらかじめ関係グラフとして構築}し，入力画像に対しては
\textbf{物体認識によって得られる観測情報を手がかりにグラフ探索を行う}点にある．

具体的には，まず物体ペアに対する関係語・位置関係・接触関係をLLMにより大量生成し，
それらの出現頻度を用いて関係グラフを構築する．
次に，入力画像から物体候補の列挙と座標推定を行い，
得られた物体と位置情報を探索の起点として関係グラフを探索することで，
物理的・空間的に整合する三つ組〈主語，関係，目的語〉を推定する．
最後に，推定された三つ組を統合し，画像に対応するシーングラフを構成する．

提案手法の処理フローを図\ref{fig:method_overview}に示す．

\section{グラフ構築}
本節では，入力画像とは独立に，物体間関係に関する知識を関係グラフとして構築する方法を述べる．
関係グラフは後段のシーングラフ生成における探索空間として用いられ，
画像から得られる観測情報（物体名・位置関係・接触情報）に整合する関係推論を可能にする．

グラフ構築は大きく二段階から成る．
第一に，物体ペアに対してLLMを用いて関係語を生成し，関係語に加えて位置関係（vertical/horizontal/depth）および
接触関係（attach）を含む関係データを得る（\ref{sec:relation_generation}節）．
第二に，得られた関係データを集計し，物体・位置付き物体・関係表現・三つ組を結ぶ多層構造の有向グラフとして
関係グラフを構築する（\ref{sec:relation_graph_construction}節）．

\subsection{関係語生成}\label{sec:relation_generation}
本ステップでは，物体語彙集合 $\mathcal{V}$ に対して，全ての物体ペア $(o_i, o_j) \in \mathcal{V} \times \mathcal{V}$ を対象とし，それぞれの関係を大規模言語モデルによって生成する．
生成結果は，関係語（relation）に加え，位置関係（position）および接触関係（attach）を含むJSON形式で出力される．
...
そこで本研究では，後段の探索において画像から得られる位置情報を有効に利用できるよう，LLMに対して文章生成と同時に2物体間の空間的な位置関係を明記させるように設計する．これにより，「apple」と「table」のような語彙的関係にとどまらず，空間的に整合する構造的情報を持つようになる．
...
\subsubsection{接触関係の生成}\label{attachment}
...
\subsection{関係グラフ構築}\label{sec:relation_graph_construction}
...
本研究における関係グラフの重み設計は，LLMが人間によって書かれた大量の文章を学習しており，常識的・経験的に妥当な関係性ほど高頻度で生成されるという性質に基づいている．物体ペアに対して関係文を大量に生成させ，その中で頻出する関係性ほど信頼度が高いと仮定し，出現頻度に基づいて関係グラフのエッジを定義する．
...
\label{attachment}で生成された"attach"は，生成文が想定する状況における物理的接触の有無を表す．グラフ構築の際，この情報をtripletノードの属性として付与する．
...
提案手法では，前節で構築した関係グラフを固定の知識基盤として用い，
画像から得られる観測情報を探索の手がかりとして関係推論を行う．
具体的には，
（1）物体認識により画像内の物体集合とその位置情報を推定し，
（2）推定された物体ペアの相対位置・接触判定を探索条件として関係グラフを探索することで，
各物体ペアに対する関係を決定する．
得られた三つ組を統合することで，画像に対応するシーングラフを生成する．

\subsection{物体認識}
本手法では，まず入力画像から物体候補を列挙し，続いて各物体の位置情報を推定する．
物体候補の列挙にはLLMを用い，画像内に存在しうる主要物体名をオープンボキャブラリに抽出する．
次に，列挙された物体名をテキストクエリとしてGrounding DINOに入力し，
各物体のバウンディングボックスを推定することで，画像内の物体位置を得る．
さらに，物体領域をより明確に抽出するため，SAMによるマスク生成を併用し，
重なり合う物体や境界が曖昧な領域に対してもピクセルレベルでの物体形状を取得する．
...
\subsection{グラフ探索}
本ステップでは，入力画像から得られた2物体の座標情報（第項）を手がかりとして，事前に構築された関係グラフを探索し，物体間の意味的関係を推定する．グラフ探索に用いる入力情報は，物体認識段階で得られる物体名に加えて，2物体間の相対位置関係および物理的接触関係である．これらは関係グラフ上の探索起点および探索制約として利用され，画像内の幾何構造と整合する関係推論を可能にする．以下では，相対位置関係と接触関係の決定方法，およびそれらを用いた関係グラフ探索の手順について説明する．
...
\subsection{シーングラフ生成}
本節では，前段のグラフ探索により推定された関係性を用いて，
入力画像に対するシーングラフを構築・可視化する手順を述べる．
提案手法では，画像内の各物体をノードとし，
物体間の推定関係をエッジとして付与することで，
画像の構造をグラフとして表現する．
このグラフは，後段の解析や可視化，
および他モジュールへの入出力（GraphML等）を想定した形式で保存される．


\subsubsection{入力データ形式}
シーングラフ生成の入力は，
物体認識で得られた各物体情報に対して，
グラフ探索によって推定された関係性を付与したJSONである
（object\_relation\_data.json）．
各物体はIDを持ち，
中心座標（center\_point）と，
推定関係の集合（relations）を持つ．
relationsは

\begin{itemize}
    \item 自分ID \texttt{|} relation \texttt{|} 相手ID : score
\end{itemize}
...
\subsubsection{シーングラフ構築}
シーングラフの構築では，
前段の関係推論によって得られた物体ごとの関係情報を入力として，
画像内の各物体をノードとするグラフを生成する．
...
属性として中心座標を保持する．
中心座標は可視化や描画時に利用されるが，
GraphML形式ではタプルが扱いづらい場合があるため，
保存時には数値属性（pos\_x, pos\_y）として別途保持し，
タプル形式の属性は削除している．
...
\begin{itemize}
    \item 始点：主語側の物体ノード
    \item 終点：目的語側の物体ノード
    \item ラベル：\texttt{relation\_score}（例：\texttt{on\_0.92}）
\end{itemize}
...
推定された関係語とその確からしさが，
グラフ構造として明示化される．
本研究ではNetworkXを用いてグラフを構築し，
最終的にGraphMLとして保存することで，
汎用的なグラフ処理ツールや可視化ツールで
再利用できる形式にしている．
