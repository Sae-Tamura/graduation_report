...
1関係語生成→2.構築
\subsection{関係語生成}
本ステップでは，物体集合$\hat{O} = \{\hat{o}_1, \hat{o}_2, \ldots, \hat{o}_n\}$に対して，全ての物体ペア$(o_i, o_j)$を組み合わせ，それぞれの関係を大規模言語モデルによって生成する．生成結果は，関係語（relation）に加え，位置関係（position）および接触関係（attach）を含むJSON形式で出力される．
